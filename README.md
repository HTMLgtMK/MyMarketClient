...
1. 修改了会员头像上传逻辑，若系统版本小于5.0，则裁剪后上传，否则直接上传。

2. 修改了二维码扫描器。测试发现相机预览的像素太低，以致于不能正确的扫描二维码。
	添加了`findBestPreviewSizeValue`方法， 添加了 `AutoFocusManager` 管理器。
	目前可以实现相机自动对焦，可以正确的扫描二维码。

-------------------------------------------------

2018.05.25 20:32

1. 添加了开机动画, 将登陆逻辑检查放在了开机界面。

2. 修改了Refresh动画, 使用AnimationUtil加载动画。
...
1. 完成余额支付内容。
	1. 新建控件 `PayPasswordInput`用于密码输入, **存在输入时宽度变化的问题**.
	2. 余额支付链接格式: `market://balancePay/token`
...
2. 修改了会员数据和Token数据的获取。
	不再使用`Application`存储, 原因是当应用出现内存不足或者崩溃时， 
	Application会重新创建，而此时的Application Instance不再是之间保存数据的实例。
	
3. 修改了用户相关数据请求方法, 使用抽象方法`getUserToken()`获取用户Token, 也可以直接传入，
	但是我就是感觉使用abstract method的逼格高一点。

4. 加入了[ACache](https://github.com/yangfuhai/ASimpleCache)项目用于做二级缓存，
	为了尽量快的完成工作，舍弃了另外一个磁盘缓存项目`DiscLruCache`，
	另外， `Volley`本身也会使用磁盘缓存，但是目前没有搞清楚是在什么时候会缓存:)
